...
postsRouter.get("/create-post", auth, (req, res) => {
  try {
    ...
    return res.json({
      status: "failure",
      msg: "Student cannot create Blog posts.",
    });
  } catch (err) {
...
postsRouter.post(
  ...
  auth,
  [
    check("title", "Post title should not be blank.").not().isEmpty(),
    check("visibility", "Please Select the visibility options").not().isEmpty(),
  ],
  async (req, res) => {
    try {
      const user = req.user;
      ...
      return res.json({
        status: "failure",
        msg: "Student cannot create Blog Post.",
      });
    } catch (err) {
      return res.json({ status: "error", message: "We caught an error" });
    }
...
postsRouter.get("/edit/:post_id", auth, async (req, res) => {
  try {
    ...
    let isMyPost = await Posts.findOne({
      _id: post_id,
      created_by: user.user_id,
      deleted_at: "Null",
    });
    ...
    if (!isMyPost) {
      return res.json({
        status: "failure",
        msg: "You cannot edit post published from another account",
      });
    }
...
postsRouter.put(
  ...
  async (req, res) => {
    try {
      ...
      let postData = await Posts.findOneAndUpdate(
        { _id: post_id, created_by: user.user_id, deleted_at: "Null" },
        {
          $set: {
            title: title,
...
postsRouter.get("/public", async (req, res) => {
  try {
    ...
    if (posts.length > 0) {
      res.json({
        status: "success",
        msg: "You are viewing all posts which are marked visible to Public",
        posts: posts,
      });
    }
    return res.json({
...
postsRouter.get("/private", auth, async (req, res) => {
  try {
    const user = req.user;
    if (!user) {
      let posts = await Posts.find({
    ...
    return res.json({
      status: "success",
      msg: "Showing Public and Private Posts",
      posts: posts,
    });
  } catch (err) {
    return res.json({ status: "error", message: "We caught an error" });
...
postsRouter.delete("/delete/:post_id", auth, async (req, res) => {
  try {
    const user = req.user;
    const post_id = req.params.post_id;
    const isMyPost = await Posts.find({
      _id: post_id,
      created_by: user.user_id,
    });
    if (isMyPost) {
      const deleted = await Posts.updateOne(
        { _id: post_id },
    ...
    return res.json({
      status: "failure",
      msg: "Post was not created by the loggedin user",
    });
  } catch (err) {
    return res.json({ status: "error", message: "We caught an error" });
  }
