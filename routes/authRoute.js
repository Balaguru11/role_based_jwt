...
mainRouter.post(
  ...
  [
    ...
    check("username", "Username should be lowercase.").isLowercase(),
    check(
      "password",
    ...
    check(
      "mobile",
      "Not a vaild Mobile number. Mobile number should be 10 characters in length."
    )
      .isNumeric()
      .isLength(10),
  ...
  try {
    ...
    if (userAcc) {
      ...
      if (userAcc.verify_status == "Pending") {
        ...
        const token = jwt.sign({ role, user_id }, process.env.TOKEN_KEY, {
          expiresIn: "2h",
        });
        userAcc.token = token;
        ...
        return res
          .status(201)
          .json({ status: "success", msg: "User verified." });
      } else {
...
mainRouter.post("/resend-verification-mail", async (req, res) => {
  try {
    ...
    if (findUser) {
      ...
      const newVerifyCode = Math.random()
        .toString(36)
        .slice(2, 8)
        .toUpperCase();
      const updateAccStatus = await User.updateOne(
        { _id: user_id },
        { $set: { verify_g: newVerifyCode } }
      );
      if (updateAccStatus) {
        ...
        const newVerifyMail = await pushMail({
          ...
          .then((result) => {
            return res.json({
              status: "success",
              msg: "Verification code has been sent to your mail id.",
            });
          })
          .catch((err) => {
            return res.json({
  ...
  try {
    ...
    if (oldUser && oldUser.verify_status == "Verified") {
      ...
      if (passMatch) {
        ...
        return res.json({
          status: "fail",
          msg: "No user match with the provided credential",
        });
      }
...
mainRouter.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const hasedVerifyCode =
        bcrypt.hashSync(reversedMail, 10) + "&&" + isAcc._id;
      ...
      const emailVerify = await pushMail({
        ...
        .then((result) => {
          return res.json({
            status: "success",
            message: "Your email verification code has been sent.",
            encodedHashCode,
          });
        })
        .then((result) => {
        ...
        .catch((err) => {
          return res.json({
            status: "error",
            messsage: "Error sending email verification code.",
            err,
          });
        });
    } catch (error) {
...
mainRouter.get("/reset-password/:verify_code", async (req, res) => {
  try {
    ...
    if (!compareHash) {
      return res.json({
        status: "error",
        message: "Url validation failed. Not a valid URL.",
      });
    }
    ...
    try {
      ...
      if (new_pwd == new_match_pwd) {
        const updatePwd = await User.updateOne(
          { _id: user_id, deleted_at: "Null", verify_status: "Verified" },
          { $set: { password: new_pwd } }
        );
        if (updatePwd) {
          return res.json({
            status: "success",
            message: "Kudos, Your Account Password has been updated.",
          });
        }
        return res.json({
          status: "failure",
...
mainRouter.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const pwdResetSecret =
        findAcc.password + "-" + findAcc.createdAt.getTime();
      ...
      const emailVerify = await pushMail({
        ...
        .then((result) => {
          return res.json({
            status: "success",
            message:
              "A link to Reset your account Password has been mailed to you.",
            resetToken,
          });
        })
        .then((result) => {
...
mainRouter.post(
  "/reset-password/:user_id/:resetToken",
  [
    check(
      "new_pwd",
      "Password must be 8 characters in length and should contain special characters as well."
