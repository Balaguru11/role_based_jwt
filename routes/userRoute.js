...
userRoute.get('/my-profile', auth, async(req, res) => {
    try {
        ...
        if(user) {
            ...
            return res.json({status: 'notfound', message: 'Please create your Profile'}).render('components/profile-create');
...
userRoute.get('/update-profile', auth, async (req, res) => {
    try {
        const user = req.user;
        const profileData = await User.findOne({_id: user.user_id, deleted_at: 'Null'});

        if(!profileData) {
            return res.json({status: 'failure', message: 'Profile not found.'})
        }
        // form with data
        return res.json({status: 'success', message: 'Showing your Profile data in a Edit Form', profileData: profileData})
    } catch (err) {
        console.log(err);
    }
})

userRoute.put('/update-profile', auth, [check('full_name', 'Please enter your name').not().isEmpty(), check('gender', 'Please Select your Gender').not().isEmpty(), check('dob', 'Enter Date of Birth.').not().isDate(), check('education', 'Fill your educational qualification').not().isEmpty(), check('about_me', 'Let us know more, say a few words about you').not().isEmpty() ], async (req, res) => {
    try {
        const user = req.user;
        if(!user){
            return res.json({status: 'failure', message: 'User not logged in.'})
        ...
        const updateProfile = await User.updateOne({_id: user.user_id, deleted_at:'Null'}, {$set: {full_name: full_name, gender: gender, dob: dob, education: education, website: website, about_me: about_me}})
        if(updateProfile) {
            return res.json({status: 'success', message: 'Profile updated.', updateProfile})
        }
